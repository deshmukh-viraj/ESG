...
def load_data(params: dict) -> pd.DataFrame:
    ...
    try:
        if all(k in params["data_ingestion"] for k in ("s3_bucket", "aws_access_key", "aws_secret_key", "s3_file_key")):
            logger.info("Loading data from AWS S3....")
            s3 = s3_operations(
        ...
        logger.error(f"Failed to laod data: {e}")
        raise
    ...
    try:
        ...
        if drop_cols:
            ...
            if missing_strategy == "drop":
                ...
                logger.info("Filled missing values with ")
            elif missing_strategy == "median":
                df.fillna(df.median(), inplace=True)
                logger.info("Filled missing values with median")
            elif missing_strategy == 'mode':
                df.fillna(df.mode().iloc[0], inplace=True)
                logger.info("Filled missing values with columns modes")
            logger.info(f"preprocessing complete")
            return df
        
    except Exception as e:
        logger.error(f"Error During preprocessimg: {e}")
        raise
